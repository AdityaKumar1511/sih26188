...
def test_validators():
    ...
    print(f" [PASS] Valid Aadhaar passed: {valid_aadhaar}")
    ...
    print(f" [PASS] Tampered Aadhaar caught: {invalid_aadhaar} -> {res2['error']}")

    # Invalid starting digit (starts with 0 or 1)
    bad_prefix = "048921049812"
    res3 = validate_aadhaar_number(bad_prefix)
    assert res3["is_valid"] is False
    print(f" [PASS] Bad prefix Aadhaar caught: {bad_prefix} -> {res3['error']}")
    ...
    print(f" [PASS] Valid PAN format checked: {valid_pan} (Warning caught: {p_res1['warning']})")
    ...
    print(f" [PASS] PAN with matching surname initial passed: {pan_matching} (5th char 'S' matches 'SHARMA')")

    # Invalid PAN format
    bad_pan = "ABCD12345F"
    p_res3 = validate_pan_number(bad_pan)
    assert p_res3["is_valid"] is False
    print(f" [PASS] Malformed PAN rejected: {bad_pan}")
...
def test_blockchain_ledger():
    ...
    print(f" [PASS] Anchored Block #{record['block_number']} | Tx: {record['tx_hash'][:18]}...")
    ...
    print(f" [PASS] Independent Audit Verified: Tx matches immutable block #{verify_res['block_number']}")
